...
def test_user_verify__unconfrimed_invalid_token(client, list_users, test_db):
    ...
    client.post(
        "/auth/login",
        data={"username": user.username, "password": "cat"},
        follow_redirects=False,
    )
...
def test_user_verify__confirmed_returns_200(client, list_users, test_db):
    *_, confirmed_user = list_users
    ...
    client.post(
        "/auth/login",
        data={"username": confirmed_user.username, "password": "rabbit"},
        follow_redirects=False,
    )
...
def test_admin_page__logged_in_but_not_admin_redirected_to_login(
    client, test_db, list_users
):
    non_admin, *_ = list_users
    client.post(
        "/auth/login",
        data={"username": non_admin.username, "password": "cat"},
        follow_redirects=True,
    )
    response = client.get("/auth/admin", follow_redirects=True)
    assert response.status_code == 200
