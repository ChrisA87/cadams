...
def test_StockPrice_process_price_data__multi_and_symbol_raises_ValueError(
    app, test_db
):
    start_date = StockPrice.get_latest_date("CADM")
    data = generate_mock_price_data(["CADM", "TEST"], start_date)
...
def test_StockPrice_process_price_data__single_and_no_symbol_raises_ValueError(
    app, test_db
):
    start_date = StockPrice.get_latest_date("CADM")
    data = generate_mock_price_data("CADM", start_date)
...
def test_StockPrice_update(app, test_db, monkeypatch):
    monkeypatch.setattr(
        StockPrice,
        "get_price_data",
        lambda *args, **kwargs: generate_mock_price_data(*args, **kwargs),
    )
