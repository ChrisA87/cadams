...
class Strategy:
    ...
    def _set_position_term(self):
        self.df["position_term"] = (
            self.df.position.where(self.df.position.ne(self.df.position.shift()))
    ...
    def _get_plot_title(self, stock):
        return (
            f"{stock.name} ({stock.symbol}) Returns | "
    ...
    def get_returns_plot_components(self, stock):
        ...
        source = ColumnDataSource(
            self.df[["returns", "strategy"]].dropna().cumsum().apply(np.exp)
        )
        p.line(
            x="date",
            y="returns",
            source=source,
            legend_label="Bought and held",
            line_width=2,
        )
        p.line(
            x="date",
            ...
            source=source,
            color="green",
            legend_label="Traded with strategy",
            line_width=2,
        )
...
class SMA(Strategy):
    ...
    def _set_moving_avgs(self, fast, slow, metric):
        if fast >= slow:
            raise ValueError(
                "Fast moving must be less than slow moving average."
    ...
    def _set_position(self):
        ...
        self.df["position"] = np.where(
            self.df["sma_fast"] > self.df["sma_slow"], 1, self.short_pos
        )
        self._dropna()
    ...
    def _get_sma_plot_title(self, stock):
        ...
        return (
            f"{stock.name} ({stock.symbol}) - {metric} with {self.fast} and {self.slow} day Moving Averages | "
            f'{self.df.index.min().strftime("%b-%Y")} - {self.df.index.max().strftime("%b-%Y")}'
        )
    ...
    def get_sma_plot_components(self, stock):
        ...
        p.line(
            x="date", y="adj_close", source=source, legend_label="Price", line_width=2
        )
        p.line(
            x="date",
            y="sma_fast",
...
class Momentum(Strategy):
    ...
    def _set_position(self):
        self.df["position"] = np.where(
            self.df["returns"].rolling(self.period).mean() > 0, 1, self.short_pos
        )
        self._dropna()
...
class MeanReversion(Strategy):
    ...
    def _set_position(self):
        ...
        self._set_distance()
        self.df["position"] = np.where(
            self.df["distance"] > self.threshold, self.short_pos, np.nan
        )
        self.df["position"] = np.where(
            self.df["distance"] < -self.threshold, 1, self.df["position"]
        )
        self.df["position"] = np.where(
            self.df["distance"] * self.df["distance"].shift(1) < 0,
            0,
            self.df["position"],
        )
    ...
    def get_distance_plot_components(self, stock):
        ...
        p.line(
            ...
            y="distance",
            source=source,
            legend_label="Distance from mean",
            line_width=2,
        )
        upper_threhold = Span(
            location=self.threshold, dimension="width", line_color="red", line_width=1
        )
        lower_threhold = Span(
            location=-self.threshold, dimension="width", line_color="red", line_width=1
        )
...
class OLS(Strategy):
    ...
    def _set_position(self):
        self._set_lags()
        reg = np.linalg.lstsq(
            a=self.df[self.lag_cols], b=np.sign(self.df["returns"]), rcond=None
        )[0]
